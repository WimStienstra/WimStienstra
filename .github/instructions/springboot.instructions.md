...
### Data Access (Prisma & GraphQL)

- **Prisma**: If using Prisma with Java (via a community client), follow the generated client's patterns for data access. Ensure the `schema.prisma` is the source of truth.
- **GraphQL**:
  - Prefer GraphQL controllers (`@Controller` with `@QueryMapping`/`@MutationMapping` in Spring for GraphQL) over REST `@RestController`.
  - Map GraphQL types to internal DTOs or Entities carefully.
  - Use DataLoaders to solve N+1 problems in GraphQL resolvers.
